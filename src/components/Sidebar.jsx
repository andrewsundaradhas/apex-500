...
const NAV_ITEMS = [
  { id: 'dashboard',    path: '/dashboard',    icon: 'dashboard', label: 'Dashboard',   kbd: 'D' },
  { id: 'predictions',  path: '/predictions',  icon: 'sparkles',  label: 'Predictions', kbd: 'P' },
  { id: 'insights',     path: '/insights',     icon: 'activity',  label: 'Insights',    kbd: 'I' },
  { id: 'watchlist',    path: '/watchlist',    icon: 'star',      label: 'Watchlist',   kbd: 'W' },
];
...
function NavItem({ icon, label, kbd, active, collapsed, onClick }) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      ...
      {active && (
        <span style={{
          position: 'absolute', left: collapsed ? 6 : -1, top: 7, bottom: 7,
          width: 2, background: 'var(--accent-primary)', borderRadius: 2,
        ...
        <>
          ...
          {kbd && (
            <span style={{
              ...
              background: 'var(--bg-overlay)', border: '1px solid var(--border-subtle)', borderRadius: 3,
            }}>{kbd}</span>
          )}
        </>
      )}
  ...
  return (
    <aside style={{
      ...
      <div style={{
        ...
        background: 'linear-gradient(180deg, var(--purple-tint-2) 0%, transparent 100%)',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--fg-primary)', cursor: 'pointer' }}
          onClick={() => navigate('/dashboard')}>
          <svg width="24" height="24" viewBox="0 0 40 40" fill="none">
            <defs>
              <linearGradient id="sbLogo" x1="6" y1="30" x2="34" y2="10">
                <stop offset="0" stopColor="#8B5CF6"/><stop offset="1" stopColor="#818CF8"/>
              </linearGradient>
            </defs>
      ...
      <nav style={{ padding: 8, display: 'flex', flexDirection: 'column', gap: 2, flex: 1 }}>
        {NAV_ITEMS.map(it => (
          <NavItem key={it.id} {...it} active={active === it.id} collapsed={collapsed}
            onClick={() => navigate(it.path)} />
        ))}
        {!collapsed && (
          <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--fg-tertiary)', padding: '14px 10px 4px', fontWeight: 500 }}>
            Models
          </div>
        )}
        {collapsed && <div style={{ height: 1, background: 'var(--border-subtle)', margin: '12px 8px' }} />}
        {MODEL_ITEMS.map(it => (
          <NavItem key={it.id} {...it} active={false} collapsed={collapsed} onClick={() => {}} />
        ))}
      </nav>

      {/* Footer */}
      <div style={{ padding: 8, borderTop: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', gap: 2 }}>
        <NavItem id="settings" icon="settings" label="Settings" collapsed={collapsed}
          active={active === 'settings'} onClick={() => navigate('/settings')} />
        <button onClick={onToggle} style={{
          background: 'transparent', border: 'none', cursor: 'pointer',
          padding: '8px 10px', borderRadius: 'var(--radius-sm)', color: 'var(--fg-tertiary)',
          display: 'flex', alignItems: 'center', gap: 10,
          justifyContent: collapsed ? 'center' : 'flex-start',
        }}>
